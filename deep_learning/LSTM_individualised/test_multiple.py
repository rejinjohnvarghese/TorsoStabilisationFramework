...
for model_comb, test_files in zip(models_comb, test_comb):
    Y, Z = model_comb
    model_path = f'model_paper_{Y}_{Z}.h5'
    model = load_model(model_path)
    
    for test_file in test_files:
        # Load and normalize the test data
        data_path = f"labelled/labelled_{test_file}.csv"
        ...
        results_file_path = f'pred_paper_{test_file}_from_{Y}_{Z}.csv'
