...
def plot_predicted_labels_with_lead_times(file_path, raw_file_path=None):
    ...
    if not raw_df.empty:
        ...
        start_of_pre_falls = []
        lead_times_label_method = []
        lead_times_threshold_method = []
        ...
        for start_index in start_of_pre_falls:
            ...
            if len(subsequent_minimal_peaks) > 0:
                ...
                lead_time_label_method = (timestamps[next_minimal_peak_index] - timestamps[start_index]) * 1000  # Convert to milliseconds
                if lead_time_label_method < 700:  # Ensure lead time is within threshold
                    lead_times_label_method.append(lead_time_label_method)
                    plt.plot([timestamps[start_index], timestamps[next_minimal_peak_index]], 
                             [acc_magnitudes[start_index], acc_magnitudes[next_minimal_peak_index]], 'b-', linewidth=2)
                    # Annotate with lead time for blue lines (label method)
                    plt.annotate(f'{lead_time_label_method:.0f}ms', 
                                 (timestamps[start_index], acc_magnitudes[start_index]), 
                                 textcoords="offset points", xytext=(-40,30), ha='center', color='blue', fontsize=25)
                    
                    # Starting from the first "pre fall" class point, find the first point below the threshold
                    for j in range(start_index, len(acc_magnitudes)):
                        if acc_magnitudes[j] < 8.8:
                            lead_time_threshold_method = (timestamps[next_minimal_peak_index] - timestamps[j]) * 1000
                            lead_times_threshold_method.append(lead_time_threshold_method)
                            plt.plot([timestamps[j], timestamps[next_minimal_peak_index]], 
                                     [acc_magnitudes[j], acc_magnitudes[next_minimal_peak_index]], 'magenta', linewidth=2)
                            # Annotate with lead time for light blue lines (threshold method)
                            plt.annotate(f'{lead_time_threshold_method:.0f}ms', 
                                         (timestamps[j], acc_magnitudes[j]), 
                                         textcoords="offset points", xytext=(-60,-50), ha='center', color='magenta', fontsize=25)
                            break
    ...
    plt.xlim(36.5, 39.0)
    plt.title('Predicted Labels with Lead Times', fontsize=30)
    plt.grid(True)
