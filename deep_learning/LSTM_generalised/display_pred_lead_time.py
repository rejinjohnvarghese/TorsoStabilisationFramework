...
def plot_predicted_labels_with_lead_times(file_path, raw_file_path=None):
    ...
    if not raw_df.empty:
        ...
        minimal_peaks, _ = find_peaks(inverted_acc_magnitudes, height=-8)
            ...
            if len(subsequent_minimal_peaks) > 0:
                ...
                if lead_time < 700:  # Check if lead time is less than 500ms
                    ...
                    plt.annotate(f'{lead_time:.2f}ms', 
                                 ...
                                 textcoords="offset points", xytext=(-30,30), ha='center', color='blue', fontsize=20)
...
file_path = 'pred_paper_9.4_from_generalized.csv'
raw_file_path = '../LSTM_individual/labelled/labelled_9.4.csv'
