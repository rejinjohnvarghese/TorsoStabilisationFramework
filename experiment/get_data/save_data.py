...
filename_csv = f"raw/{NUMBER}_raw_full.csv"
filename_video = f"videos/{NUMBER}_video.avi"

# Open the CSV file to store data
with open(filename_csv, mode='w', newline='') as file:
    file.write("timestamp,acceleration_x,acceleration_y,acceleration_z,gyro_x,gyro_y,gyro_z,magnetometer_x,magnetometer_y,magnetometer_z\n")

async def send_threshold_continuously(client):
    while True:
        try:
            # Get threshold input from user asynchronously
            threshold = await aioconsole.ainput("Enter threshold value (or 'exit' to quit): ")
            if threshold.lower() == 'exit':
                break

            # Convert the float threshold to bytes
            threshold_bytes = struct.pack('f', float(threshold))
            await client.write_gatt_char(THRESHOLD_CHARACTERISTIC_UUID, threshold_bytes)
            print(f"Threshold {threshold} sent successfully")
...
def notification_handler(sender, data):
    ...
    with open(filename_csv, mode='a', newline='') as file:
        file.write(f"{values[0]},{values[1]},{values[2]},{values[3]},{values[4]},{values[5]},{values[6]},{values[7]},{values[8]},{values[9]}\n")
...
async def run(video_recorder):
    ...
    for device in devices:
        if device.name == "Torso Stabiliser":  # Adjust the name based on your ESP32 device name
            ESP32_device = device
            break
